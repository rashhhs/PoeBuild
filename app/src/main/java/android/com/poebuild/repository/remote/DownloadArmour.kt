...
import android.com.poebuild.repository.remote.data.ArmourQuery
import android.com.poebuild.repository.remote.data.POEQuery
import io.reactivex.rxjava3.core.Single
import io.reactivex.rxjava3.schedulers.Schedulers

class DownloadArmour(val service:DownloadServiceInterface):DownloadLayer {
    ...
    fun getDownload(offset: Int, limit: Int, lastQuerySize: Int, errorQuery: Throwable?){
        ...
        if(errorQuery == null && (lastQuerySize > 0 || offset == 0)){
            executeObservable(offset, limit,
                service.getArmours(limit.toString(),offset.toString(),"json"))
        }
    }

    fun executeObservable(offset:Int, limit:Int, observable: Single<ArmourQuery>){
    ...
    override fun <T : POEQuery<T>> store(values: T) {
        val armour = Armour()
        if(values is ArmourQuery)
            armour.insert(values.query.map { wrapper -> wrapper.wrapp })
    }
